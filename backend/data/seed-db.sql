...
UNION ALL SELECT 
  'Outbreak Ops',
  'Équipez-vous et choisissez votre camp : humains ou zombies. Zones de combat post-apocalyptiques, lumière noire, effets de fumée et scoring en temps réel sur bracelets connectés.',
  5, 0, false, 'outbreak_ops.png', l.id, c.id
FROM level l, category c WHERE l.value = 2 AND c.name = 'Instinct de survie'
UNION ALL SELECT 
  'Target Panic',
  'Un stand futuriste où vous devez viser des têtes de zombies mécaniques avec des pistolets lumineux. Capteurs, sons délirants et effets LED à chaque tir réussi.',
...
UNION ALL SELECT 
  'Infection Zero',
  'Explorez le laboratoire où tout a commencé. Résolvez des énigmes, activez des machines, évitez les contaminations… et survivez jusqu’à la désactivation du virus.',
...
UNION ALL SELECT 
  'Neon Virus',
  'Plongez dans un monde virtuel électro-zombie : combattez des hordes numériques, esquivez des éclairs verts et traversez un univers psychédélique au rythme d’une bande-son techno.',
  4, 0, false, 'neon_virus.png', l.id, c.id
FROM level l, category c WHERE l.value = 3 AND c.name = 'Réalité inhumaine'
UNION ALL SELECT
  'The Last Transmission',
  'Vous recevez un dernier message d’un survivant. Suivez sa trace en VR à travers des tunnels, drones infectés et pièges lumineux. Suspense, humour noir et adrénaline.',
...
UNION ALL SELECT 
  ...
  9, 0, false, 'rave_of_the_dead.png',
  l.id, c.id
FROM level l, category c WHERE l.value = 1 AND c.name = 'Freak shows';
UNION ALL SELECT 
  'Zombie Glam Lab',
...
UNION ALL SELECT 
  'Undead Memories',
  'Scènes photo interactives : bus abandonné, portail du bunker, laboratoire contaminé… et des zombies farceurs qui posent avec vous. Effets UV et humour décalé.',
